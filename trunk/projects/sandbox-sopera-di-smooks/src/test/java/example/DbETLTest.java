...
public class DbETLTest extends TestCase {
	...
	public void test() throws Exception {
		...
		try {
			...
			assertEquals(
					"{ORDERNUMBER=1, USERNAME=user1, STATUS=0, NET=59.97, TOTAL=64.92, ORDDATE=2006-11-15}",
					orders.get(0).toString());
			assertEquals(
					"{ORDERNUMBER=2, USERNAME=user2, STATUS=0, NET=81.3, TOTAL=91.06, ORDDATE=2006-11-15}",
					orders.get(1).toString());
			assertEquals(4, orderItems.size());
			assertEquals(
					"{ORDERNUMBER=1, QUANTITY=1, PRODUCT=364, TITLE=The 40-Year-Old Virgin, PRICE=28.98}",
					orderItems.get(0).toString());
			assertEquals(
					"{ORDERNUMBER=1, QUANTITY=1, PRODUCT=299, TITLE=Pulp Fiction, PRICE=30.99}",
					orderItems.get(1).toString());
			assertEquals(
					"{ORDERNUMBER=2, QUANTITY=2, PRODUCT=983, TITLE=Gone with The Wind, PRICE=25.8}",
					orderItems.get(2).toString());
			assertEquals(
	...
	private static void eatBigFile() throws IOException, SAXException {
		...
		try {
			FileReader reader = new FileReader("./resources/zap/big-edi.edi");

			try {
				long start = System.currentTimeMillis();
				smooks.filterSource(smooks.createExecutionContext(),
						new StreamSource(reader), null);
				System.out.println("Took: "
						+ (System.currentTimeMillis() - start));
				System.err.println("eat Big EDI ok");
	...
	private static void writeBigFile() throws IOException {
		FileWriter writer = new FileWriter("./resources/zap/big-edi.edi");
		...
		try {
			...
			System.err.println("write Big EDI ok");
		} finally {
			writer.flush();
			writer.close();
		}
